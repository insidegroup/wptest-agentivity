...
class Thrive_Theme {
	...
	private function integrations() {
		...
		/* Yoast integration */
		require_once THEME_PATH . '/integrations/yoast/class-thrive-yoast.php';
	...
	public function init() {
		...
		require_once THEME_PATH . '/integrations/woocommerce/class-thrive-woo.php';
	...
	public function enqueue_scripts() {
		...
		if ( ! empty( $template_style ) && thrive_template()->is_default() && ( ! is_singular() || ! tve_post_is_landing_page() ) && ! (
		...
		if ( Thrive_Wizard::is_frontend() ) {
			tve_dash_enqueue_script( 'ttb-wizard-preview', get_stylesheet_directory_uri() . '/inc/assets/dist/wizard.min.js', [ 'theme-frontend' ] );
			wp_enqueue_style( 'ttb-wizard', get_stylesheet_directory_uri() . '/inc/assets/dist/wizard.css' );
		}
		...
		switch ( $context ) {
			case 'admin':
				$object = [
					...
					'content_types'              => Thrive_Utils::get_content_types(),
	...
	public function add_meta_boxes() {
		...
		if ( ! $thrive_post->is_landing_page() && Thrive_Utils::is_allowed_post_type( $thrive_post->ID ) ) {
			if ( tcb_admin()->tcb_enabled( $thrive_post->ID ) ) {
				...
				if ( ! Thrive_Woo::is_admin_shop_page() ) {
					add_meta_box(
	...
	public function posts_per_page( $value ) {
		...
		if ( ! empty( $wp_query->query ) && ! is_admin() && ! TCB_Utils::is_rest() && ! is_singular() && ! TCB_Utils::in_editor_render( true ) ) {
			$sections = thrive_template()->meta( 'sections' );

			if ( ! empty( $sections ) ) {
				foreach ( $sections as $section ) {
					$posts_per_page = Thrive_Section::get_posts_per_page( $section );
