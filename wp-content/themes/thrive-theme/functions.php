...
function thrive_admin_notice( $message, $state = 'error' ) {
	add_action( 'admin_notices', function () use ( $message, $state ) {
...
/**
 * Show the theme meta description
 */
function thrive_meta_description() {
	$show_meta = apply_filters( 'thrive_theme_show_meta', true );

	if ( $show_meta ) {
		echo TCB_Utils::wrap_content( '', 'meta', '', '', [
			'name'    => 'description',
