...
class Thrive_Section_REST {
	...
	public function register_routes() {
		...
		register_rest_route( self::$namespace, self::$route . '/cloud', [
			[
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => [ $this, 'get_cloud_sections' ],
				'permission_callback' => [ $this, 'route_permission' ],
			],
		] );

		register_rest_route( self::$namespace, self::$route . '/cloud' . '/(?P<id>[A-Za-z0-9]+)', [
			[
				'methods'             => WP_REST_Server::READABLE,
				'callback'            => [ $this, 'download_section' ],
				'permission_callback' => [ $this, 'route_permission' ],
			],
		] );
	 ...
	 */
	public function download_section( $request ) {
		 ...
		 *
		 * @see Thrive_Section convert_css()
		 */
	 ...
	 */
	public function get_cloud_sections( $request ) {
		...
		switch ( $section_type ) {
			...
			case THRIVE_FOOTER_SECTION:
				...
				$params   = apply_filters( 'thrive_theme_hf_params', [
					...
					'included_cloud_fields' => [ 'skin_tag' ],
				] );
			...
			default:
				/**
				 * We need to know on which template we are because we need to show different sections for list and singular
				 *
				 * @see Thrive_Section convert_css()
				 */
				thrive_template( $request->get_param( 'template_id' ) );
	...
	public function create( $request ) {
		...
		$data = $this->prepare_section_for_insert( $request );
	 ...
	 * @param $query_vars
	 */
	private function set_query_vars( $query_vars ) {
		if ( ! empty( $query_vars ) ) {
			/** @var WP_Query */
			global $wp_query;
	 ...
	 * @return array
	 */
	private function prepare_section_for_insert( $request ) {
