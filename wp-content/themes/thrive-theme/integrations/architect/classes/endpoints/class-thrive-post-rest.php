...
class Thrive_Post_REST {
	...
	public function register_routes() {
		register_rest_route( static::$namespace, static::$route . '/(?P<post_id>[\d]+)' . static::$template_actions . '/(?P<template_id>[\d]+)', [
			[
				...
				'callback'            => [ $this, 'change_template' ],
				'permission_callback' => [ $this, 'route_permission' ],
			],
		] );

		register_rest_route( self::$namespace, self::$route . '/format/audio', [
			[
				'methods'             => WP_REST_Server::CREATABLE,
				'callback'            => [ $this, 'save_audio' ],
				'permission_callback' => [ $this, 'route_permission' ],
			],
		] );

		register_rest_route( self::$namespace, self::$route . '/format/video', [
			[
				'methods'             => WP_REST_Server::EDITABLE,
				'callback'            => [ $this, 'save_video' ],
				'permission_callback' => [ $this, 'route_permission' ],
			],
		] );

		register_rest_route( self::$namespace, self::$route . '/format/image', [
			[
				'methods'             => WP_REST_Server::EDITABLE,
	 ...
	 */
	public function change_template( $request ) {
	 ...
	 * @return WP_REST_Response
	 */
	public function save_audio( $request ) {
		$post_id = $request->get_param( 'post_id' );
	 ...
	 * @return WP_REST_Response
	 */
	public function save_video( $request ) {
		$post_id = $request->get_param( 'post_id' );
