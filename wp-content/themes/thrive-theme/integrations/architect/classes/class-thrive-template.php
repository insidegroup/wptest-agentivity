...
class Thrive_Template {
	...
	public function template() {
		if ( $this->_html === null ) {
			...
			$this->_html = TCB_Utils::wrap_content( $header . $structure . $footer, 'div', 'wrapper', $wrapper_class );
	...
	protected function get() {
		...
		$meta_query = Thrive_Utils::localize_url();
		...
		$args = [
			...
			'meta_query'     => [
				...
				[
					...
					'value' => $meta_query[ THRIVE_PRIMARY_TEMPLATE ],
				],
				[
					'key'   => THRIVE_SECONDARY_TEMPLATE,
					'value' => $meta_query[ THRIVE_SECONDARY_TEMPLATE ],
				],
				[
					'key'   => THRIVE_VARIABLE_TEMPLATE,
					'value' => $is_singular ? '' : $meta_query[ THRIVE_VARIABLE_TEMPLATE ],
				],
			],
		];
		...
		if ( $is_singular ) {
			...
			if ( $meta_query[ THRIVE_PRIMARY_TEMPLATE ] !== THRIVE_HOMEPAGE_TEMPLATE ) {
				/* homepage doesn't check for custom templates that are set on the page */
				$templates = Thrive_Utils::get_page_custom_templates();
			}

			if ( empty( $templates ) ) {

				if ( $meta_query[ THRIVE_SECONDARY_TEMPLATE ] === THRIVE_POST_TEMPLATE ) {
			...
			$args['meta_query'] = [
				...
				[
					'key'   => THRIVE_VARIABLE_TEMPLATE,
					'value' => $meta_query[ THRIVE_VARIABLE_TEMPLATE ],
				],
			];
			...
			if ( empty( $templates ) ) {
				...
				if ( empty( $templates ) ) {
					...
					/* this will return the most general archive template */
					$templates = get_posts( $args );
				}
			}
		}
		...
		if ( empty( $templates ) ) {
			$templates = Thrive_Utils::get_default_templates( $is_singular, $args );
		}
	...
	public function url( $get_content_from_cookie = false ) {
		...
		switch ( $primary_template ) {
			...
			case THRIVE_ARCHIVE_TEMPLATE:
				...
				if ( post_type_exists( $secondary_template ) && array_key_exists( $secondary_template, Thrive_Utils::get_content_types() ) ) {
					...
					if ( $secondary_template === 'product' && Thrive_Woo::active() ) {
						$url = Thrive_Woo::get_shop_url();
					} else {
			...
			case THRIVE_SINGULAR_TEMPLATE:
				...
				if ( empty( $variable_template ) || $from_tar || get_post_status( $variable_template ) !== 'publish' ) {
					$args = [
						'posts_per_page' => 1,
		...
		$url = empty( $url ) || is_wp_error( $url ) ? site_url() : $url;
	...
	public static function localize_all( $skin_id = 0 ) {
		...
		foreach ( $posts as $post ) {
			...
			$templates[] = array_merge(
				$template->export(),
				[
					'layout'      => get_the_title( $template->get_layout() ),
	...
	/**
	 * Render global styles on frontend for theme template
	 */
	public function shared_styles() {
		if ( ! is_editor_page_raw( true ) ) {
			echo tve_get_shared_styles( $this->template() );
		}
	}
	...
	/**
	 * Get tag for the skin that contains this template
	 *
	 * @return mixed|string
	 */
	public function get_skin_tag() {
		$skin_tag = '';
		$terms    = wp_get_post_terms( $this->ID, SKIN_TAXONOMY );

		if ( ! empty( $terms ) ) {
			$skin     = new Thrive_Skin( $terms[0]->term_id );
	...
	public function get_blog_query() {
		...
		if ( ! $this->is_singular() ) {
			foreach ( $this->meta( 'sections' ) as $type => $data ) {
				if ( in_array( $type, [ THRIVE_HEADER_SECTION, THRIVE_FOOTER_SECTION ], true ) ) {
					continue;
				}
