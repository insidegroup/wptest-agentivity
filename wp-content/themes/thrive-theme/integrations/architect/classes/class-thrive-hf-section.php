...
class Thrive_HF_Section {
	...
	public function render( $visibility_check = true ) {
		...
		$content = TCB_Utils::wrap_content( $content, $this->type(), 'thrive-' . $this->type(), $class, $attributes );
	...
	public static function get_all() {
		$headers_footers = [
			THRIVE_HEADER_SECTION => [],
			THRIVE_FOOTER_SECTION => [],
		];

		foreach ( get_posts( static::POST_QUERY ) as $hf ) {
	...
	public static function populate_from_cloud_template( $cloud_id, $type, $post_title, $post = null ) {
		...
		if ( ! $post ) {
			...
			update_post_meta( $symbol['id'], 'thrive_wizard_hf', $type );
			$symbol_id = $symbol['id'];
		} else {
