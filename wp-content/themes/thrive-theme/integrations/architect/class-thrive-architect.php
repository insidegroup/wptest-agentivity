...
class Thrive_Architect {
	...
	private function actions() {
		...
		add_action( 'tcb_hook_editor_footer', [ $this, 'add_widget_controls' ] );
	...
	private function filters() {
		...
		add_filter( 'tcb_remove_instances', [ $this, 'tcb_elements' ], 100 );
		...
		add_filter( 'tve_include_widgets_in_editor', [ $this, 'tve_include_widgets_in_editor' ] );
		...
		add_filter( 'tcb.post_list.disable_related', [ 'Thrive_Post_List', 'disable_query_builder_related_posts' ] );

		add_filter( 'tcb_post_list.related_text', [ 'Thrive_Post_List', 'query_builder_related_posts_text' ] );

		add_filter( 'tcb_post_list.show_exclude', [ 'Thrive_Post_List', 'query_builder_show_exclude' ] );
		...
		add_filter( 'tcb.template_path', [ $this, 'tcb_include_skin_style_panel' ], 10, 4 );
	 ...
	 * @return array
	 */
	public function tcb_elements( $used_elements ) {
		$elements_to_hide = [];

		/* first, add the elements used by other elements (example: post list, pagination button ) */
		$inherited_theme_elements = $this->get_architect_theme_elements( ARCHITECT_INTEGRATION_PATH . '/classes/inherited-elements' );

		$only_theme_elements = $this->get_architect_theme_elements();
	...
	public function editor_enqueue_scripts() {
		if ( Thrive_Utils::is_inner_frame() ) {
			...
			wp_localize_script( 'thrive-theme-editor', 'thrive_page_params', $this->localization_params() );
		}
	}
	...
	public function tcb_main_frame_enqueue() {
		...
		if ( Thrive_Utils::is_theme_template() || Thrive_Utils::allow_theme_scripts() ) {
	 ...
	 * @return array
	 */
	private function get_architect_theme_elements( $path = null ) {
		$root_path = ARCHITECT_INTEGRATION_PATH . '/classes/elements';
		...
		foreach ( $items as $item ) {
			...
			if ( is_dir( $item_path ) ) {
				$elements = array_merge( $elements, $this->get_architect_theme_elements( $item_path ) );
			}
	...
	/**
	 * Render widgets also in the theme builder
	 *
	 * @param $allow boolean
	 *
	 * @return bool
	 */
	public function tve_include_widgets_in_editor( $allow ) {

		return false;

		/* return $allow || Thrive_Utils::is_theme_template(); */
	}
	...
	public function wp_print_footer_scripts() {
		if ( Thrive_Utils::is_inner_frame() ) {
			$templates = tve_dash_get_backbone_templates( ARCHITECT_INTEGRATION_PATH . '/views/backbone/theme-editor' );
	...
	public function tcb_categories_order( $order ) {
		...
		$order[80] = Thrive_Defaults::theme_taxonomy_label();
	...
	/**
	 * Add the modal containing all the widget settings. This has to be added earlier than the other modals, so it cannot be in the modal folder.
	 */
	public function add_widget_controls() {
		if ( apply_filters( 'tve_include_widgets_in_editor', false ) ) {
			include THEME_PATH . '/inc/templates/parts/widget-settings.php';
		}
	}
	...
	/**
	 * Includes the skin style panel if the requirements are meet
	 *
	 * @param $file_path
	 * @param $file
	 * @param $data
	 ...
	 * @return string
	 */
	public function tcb_include_skin_style_panel( $file_path, $file, $data, $namespace ) {
